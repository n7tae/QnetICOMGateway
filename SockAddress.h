...
public:
	...
	bool operator==(CSockAddress &from)	// only compares the addresses, family comparison is explicit from the string representation of the address
	{									// ports are not compared
		const std::string addr(GetAddress());
		return addr.compare(from.GetAddress()) ? false : true;
	}
